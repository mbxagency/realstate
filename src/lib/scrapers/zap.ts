...
export class ZapScraper extends BaseScraper {
  protected baseUrl = 'https://www.zapimoveis.com.br';
  protected name = 'Zap Imóveis';
  ...
  protected parseProperties($: cheerio.Root): Property[] {
    ...
    // Seletor para cards de imóveis no Zap
    const propertyCards = $('.card-container');
    ...
    propertyCards.each((_, element) => {
      try {
        ...
        // Preço
        const priceText = $card.find('.card__price').text().trim() ||
        ...
        // Localização
        const location = $card.find('.card__address').text().trim() ||
                        $card.find('.address').text().trim();
        
        // Área
        const areaText = $card.find('.card__area').text().trim();
        const area = this.extractArea(areaText);
        
        // Quartos
        const bedroomsText = $card.find('.card__bedrooms').text().trim();
        const bedrooms = parseInt(bedroomsText.match(/\d+/)?.[0] || '0');
        
        // Banheiros
        const bathroomsText = $card.find('.card__bathrooms').text().trim();
        ...
        if (title && price > 0) {
          properties.push({
            id: `zap_${Buffer.from(fullLink).toString('base64').slice(0, 12)}`,
            title,
            description: title,
            price,
        ...
        console.error('Error parsing Zap property card:', error);
      }
    });
