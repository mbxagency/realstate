...
export class VivaRealInspiredScraper {
  private name = 'VivaReal (Inspired)';
  ...
  async search(query: string, filters: Partial<SearchFilters>): Promise<Property[]> {
    ...
    try {
      ...
      // Interceptar requisições para evitar bloqueios
      ...
      // Usar a URL base do VivaReal para Curitiba
      const searchUrl = 'https://www.vivareal.com.br/venda/parana/curitiba/';
      ...
      // Aguardar elementos importantes carregarem
      try {
        await page.waitForSelector('[data-type="property"]', { timeout: 10000 });
      } catch (e) {
        console.log('Seletor principal não encontrado, tentando alternativos...');
      }
      ...
      console.error('Erro no scraper inspirado do VivaReal:', error);
      return [];
    } finally {
      await browser.close();
    }
  ...
  private async extractProperties(page: puppeteer.Page): Promise<Property[]> {
    ...
    // Tentar diferentes seletores para os cards de imóveis (baseado no projeto de referência)
    const cardSelectors = [
      '[data-type="property"]',
      '.property-card',
      '.card-container',
      ...
      '.item',
      '.js-card-selector',
      // Seletores específicos do VivaReal
    ...
    for (let i = 0; i < cardsToProcess.length; i++) {
      try {
        ...
        // Extrair dados do card usando seletores específicos do VivaReal
        ...
        const bathroomsText = await this.extractText(card, [
          ...
          '[class*="bathroom"]',
          '[class*="bath"]',
          '.property-card__bathrooms-value'
        ]);
  ...
  private async extractAttribute(element: puppeteer.ElementHandle, selector: string, attribute: string): Promise<string> {
    try {
      return await element.$eval(selector, (el: any, attr: string) => el.getAttribute(attr) || '', attribute);
    } catch (e) {
      return '';
    }
  }
